...
def custom_deg_dotplot(
    ...
    z_l, pct_l = pd.melt(z_df, value_vars=[n for n in z_df.columns if n != names[0]], var_name=names[0], value_name="z-score"), pd.melt(pct_df, value_vars=[n for n in z_df.columns if n != names[0]], var_name=names[0], value_name="fraction")
